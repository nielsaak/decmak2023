...
png(file="output/posterior_estimates_monkeys.png",
...
mcmc_trace(samples_mcmc, pars = c("mu_a_rew", "mu_K", "mu_omega_p", "mu_omega_f")) +
  ...
  theme_minimal()
ggsave("output/traceplot_monkeys_mu.png", width = 10, height = 5)

samples_mcmc <- as.mcmc(samples)
mcmc_trace(samples_mcmc, pars = c("lambda_a_rew", "lambda_K", "lambda_omega_f", "lambda_omega_p")) +
  ggplot2::scale_color_discrete() +
  theme_minimal()
ggsave("output/traceplot_monkeys_lambda.png", width = 10, height = 5)
...
x_predict_saved <- array(NA, dim = c(80, 9))
...
for (s in 1:nsubs) {
  ...
  if(length(x_predict == 74)){
    for (i in 1:length(x_predict)){
      x_predict_saved[i,s] <- x_predict[i]
    }
  }
  else {
...
monkey_data <- monkey_data %>% 
  filter(species == "chimp") %>% 
  filter(!is.na(payoff)) %>% 
  select(id, session, trial, trial_new, choice, payoff)

monkey_data %>% 
  group_by(session, choice) %>%
  summarise("count" = n()) %>%
  ggplot(aes(x = as.character(session), y = count, fill=as.character(choice)))+
  geom_col()+ #Setting theme
  xlab("Session") + #Setting x label
  ylab("Count") + #Setting y label
  ggtitle("Count of choice across sessions") +
  labs(fill="Colour of choice") +
  theme_minimal() 
ggsave("output/sum_choice_session.png", width = 10, height = 5)

ifelse(x_predict_saved == t(x), 1, 0)
tibble(ifelse(x_predict_saved == t(x_all), 1, 0)) %>% 
  mutate(rowsume = rowSums(., na.rm = TRUE)) %>% 
  mutate(accuracy = rowsume/9) %>%
  mutate(idx = seq(1,80,1)) %>% 
  ggplot(aes(x = idx, y = accuracy))+
  geom_line() + #Setting theme
  xlab("Trial") + #Setting x label
  ylab("Accuracy") + #Setting y label
  ggtitle("Accuracy across trials") +
  theme_minimal() 
ggsave("output/accuracy_session.png", width = 10, height = 5)
