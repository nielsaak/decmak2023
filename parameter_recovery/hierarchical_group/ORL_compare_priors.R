...
prior_samples <- jags.parallel(data, inits=NULL, params,
                model.file ="ORL_compare_priors.txt",
...
# savage dickey plot
plot(density(rnorm(12000,0,1/sqrt(alpha_precision))),ylim=c(0,1),main=" ")
...
null.prior     <- dnorm(0,0,(1/sqrt(alpha_precision)))                   
...
fit.posterior <- logspline(prior_samples$BUGSoutput$sims.list$alpha_omega_f)
null.posterior <- dlogspline(0, fit.posterior)
...
# savage dickey plot
plot(density(rnorm(12000,0,1/sqrt(1))),xlim=c(-6,6), main="alpha")
...
# savage dickey plot
plot(density(rnorm(12000,0,1/sqrt(1))),xlim=c(-6,6),main="mu")
