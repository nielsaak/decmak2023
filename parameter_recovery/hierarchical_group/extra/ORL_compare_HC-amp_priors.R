...
prior_samples <- jags.parallel(data, inits=NULL, params,
                model.file ="extra/ORL_compare_HC-amp_priors.txt",
                n.chains=3, n.iter=5000, n.burnin=1000, n.thin=1, n.cluster=4)

end_time = Sys.time()
end_time - start_time

alpha_precision <- .1 # confer with the corresponding jags-file that the alpha precisions are set to the same value

# savage dickey plot
plot(density(rnorm(10000,0,1/sqrt(alpha_precision))),ylim=c(0,1),main=" ")
lines(density(prior_samples$BUGSoutput$sims.list$alpha_omega_p),col="red")

fit.posterior <- logspline(prior_samples$BUGSoutput$sims.list$alpha_a_rew)
...
par(mfrow=c(3,2))
plot(density(rnorm(10000,0,1/sqrt(alpha_precision))),ylim=c(0,.7),main="a_rew")
lines(density(prior_samples$BUGSoutput$sims.list$alpha_a_rew),col="red")

plot(density(rnorm(10000,0,1/sqrt(alpha_precision))),ylim=c(0,.7),main="a_pun")
lines(density(prior_samples$BUGSoutput$sims.list$alpha_a_pun),col="red")

plot(density(rnorm(10000,0,1/sqrt(alpha_precision))),ylim=c(0,.7),main="omega_f")
lines(density(prior_samples$BUGSoutput$sims.list$alpha_omega_f),col="red")

plot(density(rnorm(10000,0,1/sqrt(alpha_precision))),ylim=c(0,.7),main="omega_p")
lines(density(prior_samples$BUGSoutput$sims.list$alpha_omega_p),col="red")

plot(density(rnorm(10000,0,1/sqrt(alpha_precision))),ylim=c(0,.7),main="K")
lines(density(prior_samples$BUGSoutput$sims.list$alpha_K),col="red")
...
temp_samples <- jags.parallel(data, inits=NULL, params,
                model.file ="extra/balance_compare_HC-amp.txt",
                n.chains=3, n.iter=15000, n.burnin=1000, n.thin=3, n.cluster=4)


# savage dickey plot
plot(density(rnorm(10000,0,1/sqrt(.1))),main=" ")
