...
for (i in 1:niterations) {
  ...
  params<-c("mu_a_rew",
            "mu_K","mu_theta","mu_omega_f","mu_omega_p",
            "lambda_a_rew","lambda_K","lambda_theta",
...
pl7 <- recov_plot(true_mu_K, infer_mu_K, plot_lab_1 = expression("True "*mu[K]), plot_lab_2 = expression("Inferred "*mu[K]), 'smoothed linear fit', title=expression(mu[K])) +
...
plot_1 <- ggarrange(pl1, pl7, pl5, pl6, pl4, ncol = 2, nrow = 3)
print(plot_1)
...
pl4 <- recov_plot(true_lambda_theta, sqrt(1/infer_lambda_theta), plot_lab_1 = expression("True "*lambda[theta]), plot_lab_2 = expression("Inferred "*lambda[theta]), 'smoothed linear fit', title=expression(lambda[theta])) +
  coord_cartesian(xlim = c(0, 0.2), ylim = c(0,0.7)) 
pl7 <- recov_plot(true_lambda_K, sqrt(1/infer_lambda_K), plot_lab_1 = expression("True "*lambda[K]), plot_lab_2 = expression("Inferred "*lambda[K]), 'smoothed linear fit', title=expression(lambda[K])) +
...
plot_2 <- ggarrange(pl1, pl7, pl5, pl6, pl4, ncol = 2, nrow = 3)
print(plot_2)
...
save.image(file = "hier_ORL_recovery.RData")
