...
ntrials <- 40 # total number of trials in our payoff structure
...
ORL_sims <- ORL(payoff,ntrials,a_rew,K,theta,omega_f,omega_p)
...
params<-c("a_rew","K","theta","omega_f","omega_p")
...
for (i in 1:niterations) {
  ...
  #a_pun <- runif(1,0,1)
  K <- runif(1,0,2)
  theta <- 1 # runif(1,0,2) # could also just be a set value (e.g. 1) to simplify the model a bit
  omega_f <- runif(1,-2,2)
  omega_p <- runif(1,-2,2)
...
ggarrange(pl1, pl3, pl4, pl5, pl6)
ggsave("output/recovery_40_trials_100_iter_zscaled_theta1_fixsign.png", width = 10, height = 5)
